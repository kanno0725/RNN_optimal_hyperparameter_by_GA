...
@author: kanno
description:
    １センサーづつデータを０にして、感度解析を行う
    抜いたセンサーの番号がアウトプットに表示されている
"""
import h5py
import pandas as pd
import numpy as np
...
#データ名
dataname = 'linear_data_AveStd_coorL_s1_ntd2'
#モデル番号
model_no = '219'
...
for sd in tqdm(range(sensor_num)):
    #入出力値を読み取り
    X=array[:,1:n_in+1].astype(np.float32)
    Y=array[:,n_in+1].astype(np.int)
    
    for j in range(len(X)):
        X[j,sd*sensor_var_num:(sd+1)*sensor_var_num] = 0
    ...
    output = pd.DataFrame([['中間層','ドロップアウト率','earlystopping','参照数','ずらす時間','レイヤー','試行回数','bi-directional'],
                           [n_hiddens,p_keep,isEs,maxlen,time_bias,layer,epochs,BiDir],
                           [],
                           ['作業','溶接','移動','休憩','付帯','全体'],
                           ['作業時間(正解)(s)',it[0],it[1],it[2],it[3],it_sum],
                           ['作業時間(予測)(s)',ip[0],ip[1],ip[2],ip[3],ip_sum],
                           ['一致時間(s)',im[0][0],im[1][1],im[2][2],im[3][3],ii],
                           ['再現率(%)',ia[0]*100,ia[1]*100,ia[2]*100,ia[3]*100,np.float64(100*ii/it_sum)],
                           ['適合率(%)',ib[0]*100,ib[1]*100,ib[2]*100,ib[3]*100,np.float64(100*ii/ip_sum)],
                           ['F値',f[0]*100,f[1]*100,f[2]*100,f[3]*100,np.float64(100*ii/ip_sum)],
                           [],
                           ['比較','predict1','predict2','predict3','predict4'],
                           ['test1',im[0][0]/ip[0],im[0][1]/ip[1],im[0][2]/ip[2],im[0][3]/ip[3]],
                           ['test2',im[1][0]/ip[0],im[1][1]/ip[1],im[1][2]/ip[2],im[1][3]/ip[3]],
                           ['test3',im[2][0]/ip[0],im[2][1]/ip[1],im[2][2]/ip[2],im[2][3]/ip[3]],
                           ['test4',im[3][0]/ip[0],im[3][1]/ip[1],im[3][2]/ip[2],im[3][3]/ip[3]]
                           ])
    
    res = pd.DataFrame(['test','class'])
    for k in range(len(y_test0)):
        s = pd.Series([y_test0[k],classes[k]])
...
with pd.ExcelWriter('./sensitivity_'+str(dataname)+'.xlsx') as Writer:
    for sd2 in range(8):
        output = output_list[sd2]
        res = res_list[sd2]
        
        output.to_excel(Writer,sheet_name='sheet'+str(sd2)+'_result',index=False,header=False)
        res.to_excel(Writer,sheet_name='sheet'+str(sd2)+'_log',index=False,header=False)
